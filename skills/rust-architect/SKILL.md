...
## 16. Never Use `#[deny(warnings)]` in Library Crates
❌ **NEVER**:
```rust
// In a library crate's lib.rs
#![deny(warnings)]  // Breaks downstream on every new compiler version!
```
✅ **ALWAYS**:
```rust
// In CI / Cargo.toml [profile] or via RUSTFLAGS in CI only
// Or use specific lints: #![deny(unused_imports)]
```
**Why**: New compiler versions introduce new warnings. `#[deny(warnings)]` in a library turns those into hard errors for every downstream user, breaking their builds without any change on their part. Reserve it for application crates or enforce via CI flags, not compiled-in attributes. (Source: Rust Design Patterns — Anti-patterns)

## 17. Never Use Deref Polymorphism
❌ **NEVER**:
```rust
...
// Now MyVec "inherits" all Vec methods — this is not OOP inheritance!
```
✅ **ALWAYS**:
```rust
// Expose only the methods you intend to expose, explicitly
impl MyVec {
    pub fn len(&self) -> usize { self.0.len() }
    pub fn push(&mut self, v: i32) { self.0.push(v) }
}
```
**Why**: Deref is for smart pointers, not method inheritance. Using it for inheritance confuses callers about which type they're operating on, breaks encapsulation, and produces surprising behavior with method resolution. (Source: Rust Design Patterns — Anti-patterns)

## 18. Never Use Boolean Flags When an Enum Represents State
❌ **NEVER**:
```rust
struct Connection {
    is_open: bool,
...
enum ConnectionState {
    Closed,
    Open,
    Authenticated { tls: bool },
}
struct Connection {
    state: ConnectionState,
}
```
**Why**: Boolean flags allow impossible combinations (e.g. `is_authenticated = true, is_open = false`). Enums make invalid states unrepresentable at compile time — a core Rust design principle. (Source: corrode.dev — Using Enums to Represent State)
```
...
## API Design (6 practices) — from Rust API Guidelines

### 26. ALWAYS Follow Rust Naming Conventions
```rust
// ✅ Types and traits: UpperCamelCase; acronyms count as one word
struct HttpClient { ... }   // not HTTPClient
trait AsyncRead { ... }     // not AsyncREAD
type Uuid = [u8; 16];       // not UUID

// ✅ Values, functions, methods: snake_case
...
impl Foo {
    pub fn as_str(&self) -> &str { ... }          // cheap, borrows self
    pub fn to_string(&self) -> String { ... }     // potentially expensive, borrows self
    pub fn into_bytes(self) -> Vec<u8> { ... }    // consumes self
}
```

### 27. ALWAYS Implement Applicable Common Traits Eagerly
```rust
// ✅ For every new public type, implement all that apply:
...
// Also consider: Serialize/Deserialize, From/Into, Error, Send, Sync
```
**Why**: Traits are the Rust interoperability contract. A type missing `Debug` or `Hash` silently breaks callers who need to put it in a `HashMap` or log it. Implement eagerly; removing a trait implementation is a breaking change. (Source: Rust API Guidelines — Interoperability)

### 28. ALWAYS Use the Typestate Pattern for State Machines
```rust
...
// ❌ This won't compile — you can't call send() before authenticate()
// let conn = Connection::connect("...")?;
// conn.send(b"hello")?;  // compile error: no method send on Connection<Connected>
```
**Why**: Typestate moves runtime errors (wrong-state calls) to compile time. Zero-cost — `PhantomData` has no runtime representation. Use whenever a type has a meaningful lifecycle. (Source: Rust Design Patterns + corrode.dev)

### 29. ALWAYS Use `let-else` for Early Returns on Option/Result
```rust
// ✅ Idiomatic since Rust 1.65
let Some(user) = find_user(id) else {
    return Err(AppError::UserNotFound(id));
};
// user is in scope here, unwrapped

// ❌ Older, more verbose
let user = match find_user(id) {
    Some(u) => u,
    None => return Err(AppError::UserNotFound(id)),
};
```
**Why**: `let-else` reduces nesting, keeps the happy path at the same indentation level, and is now idiomatic Rust. Clippy enforces it. (Source: corrode.dev)

### 30. ALWAYS Use `#[non_exhaustive]` on Public Enums in Library Crates
```rust
// ✅ In library crate
#[non_exhaustive]
pub enum ApiError {
    ...
    NotFound,
    RateLimited,
    // Future variants can be added without breaking downstream match arms
}
```
**Why**: Without `#[non_exhaustive]`, adding a variant to a public enum is a breaking change — all downstream `match` expressions need a new arm. Mark public enums non-exhaustive from the start to preserve API freedom. (Source: Rust API Guidelines — Future Proofing)

### 31. ALWAYS Structure Rustdoc with Standard Sections
```rust
...
/// # Errors
///
/// Returns [`MyError::Overflow`] if the result exceeds `i64::MAX`.
///
/// # Safety  (only for unsafe fn)
///
/// Caller must ensure `ptr` is valid and aligned.
pub fn my_function(n: i64) -> Result<i64, MyError> { ... }
```
**Why**: Rustdoc renders these sections distinctly. `# Errors` and `# Panics` are the contract — missing them forces callers to read the implementation. (Source: Rust API Guidelines — Documentation)
```
...
Follow the `rust-tdd` skill for the full workflow. The cardinal rule here: **vertical slices only** — one test, then its implementation, then repeat. Never write all tests before writing any code (horizontal slicing).

### Complete Example: Adding Password Validation

Each cycle: write ONE test → run `cargo test` (RED) → write minimum code → run `cargo test` (GREEN) → next behavior.

#### Cycle 1: length check

```rust
// RED — write one test, run cargo test, watch it fail
#[cfg(test)]
mod tests {
    use super::*;
...
```rust
// GREEN — minimum code to pass this one test
use thiserror::Error;
...
#### Cycle 2: number requirement

```rust
// RED — add one more test, run cargo test, watch it fail
#[test]
fn rejects_passwords_with_no_numeric_character() {
    assert!(validate_password("password").is_err());
}
```

```rust
// GREEN — extend the error enum and add the check
#[derive(Error, Debug, PartialEq)]
pub enum PasswordError {
    #[error("Password must be at least 8 characters")]
    TooShort,
...
#### Cycle 3 (and so on): valid input confirms acceptance, edge cases, etc.

#### Refactor (after all cycles are GREEN)
...
**Run**: `cargo test` → still passes ✅ Never refactor while RED.
...
**Test Quality**
- [ ] Tests have descriptive behavior-oriented names (`rejects_password_shorter_than_eight_chars`, not `test_validate_password`)
...
**TDD Compliance**
- [ ] Vertical slices only — no horizontal slicing (no batch of tests written before any implementation)
- [ ] Tests use public interfaces only — no `pub(crate)` methods that exist solely to enable testing
- [ ] Tests would survive an internal refactor (no assertions on internal call counts or private state)
- [ ] Mocks used only at system boundaries (external APIs, time, filesystem) — own repositories use real or in-memory implementations
...
- [ ] Types and traits: `UpperCamelCase`; acronyms as one word (`Uuid` not `UUID`, `HttpClient` not `HTTPClient`)
- [ ] Functions/variables: `snake_case`
- [ ] Constants: `SCREAMING_SNAKE_CASE`
- [ ] No `get_` prefix on getters — use `.name()` not `.get_name()`
- [ ] Iterator methods: `iter()`, `iter_mut()`, `into_iter()` — not `items()`, `all_items()`, etc.
- [ ] Conversions: `as_X()` (cheap borrow), `to_X()` (potentially allocating), `into_X()` (consuming)
...
**API design (library crates):**
- [ ] Public enums marked `#[non_exhaustive]` — adding variants should not break downstream `match`
- [ ] No `#[deny(warnings)]` in library crate root — breaks downstream on compiler upgrades
- [ ] Traits that should not be implemented externally are sealed

**Type system:**
- [ ] No Deref polymorphism — `Deref` used only for smart pointers, not method inheritance
- [ ] Boolean flags replaced with enums where state combinations matter
- [ ] Typestate pattern applied for types with meaningful lifecycles (connection, transaction, builder)
- [ ] `let-else` used for early returns on `Option`/`Result` (not nested `match` or `if let`)

**Trait completeness (public types):**
- [ ] `Debug` derived/implemented on all public types
- [ ] `Clone`, `PartialEq`, `Eq`, `Hash` derived where applicable
- [ ] `Default` implemented where a sensible zero-value exists
- [ ] `Display` implemented for user-facing types

**Interface testability:**
- [ ] External dependencies injected as `&dyn Trait` or `Arc<dyn Trait>` — not constructed internally
- [ ] Functions return `Result<T, E>` rather than mutating state and returning `()`
- [ ] Module/struct interfaces are deep (small surface, rich implementation) — not shallow passthroughs

**Documentation:**
- [ ] Public functions use `# Examples`, `# Panics`, `# Errors`, `# Safety` sections as applicable
